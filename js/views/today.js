// The home screen: a scannable checklist for today — smart nudge, protein target,
// the session as tickable collapsible tasks, fuel and logging.
...
export function renderToday(container, ctx) {
  ...
  if (isTraining) {
    ...
    container.innerHTML = `
      <div id="nudges"></div>
      ${heroHTML(kicker, exact.title || info.label, info.blurb, exact, plan)}
      ...
      <div class="row-between" style="margin:22px 2px 10px">
        ...
        <span class="sess-count" id="sess-count">${tc.done}/${tc.total} done</span>
      </div>
      ${comps}
      <button class="btn btn--ghost btn--sm" id="mark-all" style="margin:2px 0 4px">${tc.all ? 'Reset all' : 'Mark all done'}</button>
  ...
  container.innerHTML = `
    <div id="nudges"></div>
    ${heroHTML(kicker, title, blurb, null, plan)}
    ${proteinPlaceholder()}
...
// ---------- Hero ----------
function heroHTML(kicker, title, blurb, day, plan) {
...
function bindTasks(container, ctx, iso, refs) {
  const refresh = () => {
    ...
    const counter = container.querySelector('#sess-count');
    if (counter) counter.textContent = `${tc.done}/${tc.total} done`;
    ...
    if (markAll) markAll.textContent = tc.all ? 'Reset all' : 'Mark all done';
    paintNudges(container.querySelector('#nudges'), ctx, iso);
  };
  ...
  container.querySelector('#mark-all')?.addEventListener('click', () => {
    const tc = taskCount(iso, refs);
    const target = !tc.all; // if not all done -> mark all; else reset
...
function mountProtein(el, ctx, iso) {
  ...
  el.innerHTML = `
    <details class="protein-more" ${wasOpen ? 'open' : ''}>
  ...
  el.querySelectorAll('[data-add]').forEach((b) => b.addEventListener('click', () => {
    ...
    paintNudges(document.getElementById('nudges'), ctx, iso);
  }));
  ...
  const addCustom = () => {
    ...
    if (v > 0) { addProtein(iso, 'Other', v); mountProtein(el, ctx, iso); paintNudges(document.getElementById('nudges'), ctx, iso); }
  };
